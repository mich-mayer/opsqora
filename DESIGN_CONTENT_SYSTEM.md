...
### 2026-07-09 numbered-kicker emphasis + footer positioning line (sync with FlatFeed)

- **Problem:** a FlatFeed design review (against an external reference landing) added two case-study refinements — larger numbered-kicker numerals and a short footer positioning line — and its own §34 entry (same date) explicitly flagged both as candidates for Opsqora parity per §5's cross-landing consistency guidance, left for a separate change. This is that change.
- **Rationale:** (1) **Numeral.** `.kicker`/`.kicker span` are byte-identical base rules between the two systems (confirmed before editing), and `--accent` is already a sanctioned usage for "index numbers" in this file's own §5.1 color table, so enlarging just the numeral inside `.case-section` reinforces an existing pattern rather than adding one. The Opsqora-specific risk this project doesn't share with FlatFeed: `Kicker` (`src/components/primitives.tsx`) is one shared component used by both the case study's numbered sections *and* the app's `ScreenHead` (Patterns/Review/Brief/Eval, `01`–`04`, §6.3) — a component-level size change would have enlarged the operational app's compact single-line head too, which §6.3 requires to stay "on one line." Scoped the rule to `.case-section .kicker span` (container-level, not component-level) — mirroring `.screen-head .kicker` already being a separate selector in the stylesheet — and verified live that `.screen-head .kicker span` (Patterns' "01") stayed at the base 0.6875rem while `.case-section`'s grew to 1.125rem. (2) **Footer line.** Condenses the human-in-the-loop boundary into a short mono caption. §20's terminology table instructs "Quote MODEL_BOUNDARY verbatim" for the AI-boundary concept, so rather than paraphrase, the line ("AI suggests patterns. The PM decides — it never self-approves.") is built from fragments already verbatim inside `MODEL_BOUNDARY` (`src/mock/index.ts`) — "the PM decides" and "never self-approves" are exact substrings — with the middle clauses (evidence validation, readiness rules) dropped for footer length, plus "AI suggests patterns" drawn from §21's own canonical verb pairing. This mirrors FlatFeed's discipline of staying verbatim-close to an existing canonical string rather than inventing new wording, adapted to Opsqora's stricter verbatim-quoting instruction for this specific concept. Reused `--ink-3` (already AA-verified ≈4.9:1, §15) and the existing `.case-foot span` mono style; the existing honesty span (`© 2026 — Phase 1 prototype…`) was preserved unchanged, only wrapped alongside the new line.
- **Affected surfaces:** `src/styles.css` (`.case-section .kicker span`; `.case-foot-meta`), `src/case-study.tsx` (footer markup — new `.case-foot-meta` wrapper holding the existing honesty span plus a new `.case-foot-line`), this file (§34). `CASE_STUDY.md`, `docs/design-direction.md`, all `src/screens/*`, `src/mock/index.ts` (including `MODEL_BOUNDARY` itself, which was read, not edited) are untouched. FlatFeed originated this pattern (its own §34 entry, same date) and was not re-touched by this change.
- **Compatibility impact:** none breaking. `npm run verify` (`tsc --noEmit && vite build`) passes; both HTML entries (`index.html`, `case-study.html`) build clean. Verified live in the dev server at 375px and 1440px: the case-section numeral renders at 19.89px / `#2236e8` (600 weight) at the 1440px root, matching FlatFeed's teal counterpart's proportions exactly; the footer's two-line stack wraps cleanly under `flex-wrap` on mobile and sits in one row on desktop with no overflow; no console errors. The one Opsqora-specific regression this change could have caused — enlarged index numbers on the Patterns/Review/Brief/Eval screen heads — was checked directly and confirmed absent (`.screen-head .kicker span` still renders at the base 12.155px).
- **Migration consideration:** fixed now, both files touched in one pass; no app-screen, mock-data, or `MODEL_BOUNDARY` changes. This closes the parity item left open by FlatFeed's `2026-07-09 hero-lede voice, numbered-kicker emphasis, footer positioning line` entry. FlatFeed's hero-lede voice change (first-person register fix, §24 verb discipline) has no Opsqora equivalent to port — Opsqora's hero is a separate copy concern, not touched here.
